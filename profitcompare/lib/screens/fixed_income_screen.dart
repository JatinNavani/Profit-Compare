...
class FixedIncomeScreen extends StatefulWidget {
  @override
  _FixedIncomeScreenState createState() => _FixedIncomeScreenState();
}

class _FixedIncomeScreenState extends State<FixedIncomeScreen> {
  final user = FirebaseAuth.instance.currentUser;
  late CollectionReference investmentsRef;

  @override
  void initState() {
    super.initState();
    investmentsRef = FirebaseFirestore.instance
        .collection('users')
        .doc(user!.uid)
        .collection('fixed_income');
  }
  ...
  void _addInvestmentDialog() {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        title: Text('Add Fixed Income Investment', style: HomeTextStyles.label),
        content: SingleChildScrollView(
          child: Column(
            children: [
              _inputField('Investment Name', nameController),
  ...
  Future<void> _deleteInvestment(String docId) async {
    await investmentsRef.doc(docId).delete();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('Fixed Income', style: HomeTextStyles.title),
        backgroundColor: Color(0xFF595CE6),
      ),
      body: StreamBuilder<QuerySnapshot>(
        stream: investmentsRef.orderBy('created_at', descending: true).snapshots(),
        builder: (context, snapshot) {
          ...
          if (docs.isEmpty) return Center(child: Text("No fixed income investments yet."));
